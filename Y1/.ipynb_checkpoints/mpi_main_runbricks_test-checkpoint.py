...
parser.add_argument('-r','--run',type=str,choices=['north','south','test'],required=True,help='Run?')
...
print(settings.legacypipe_output_dir)
    ...
    for run in tm.iterate(runcat):
        ...
        for stage,versions in run.stages.items():
            ...
            command += ['--brick',run.brickname,'--threads',threads,'--outdir',settings.output_dir,'--run',settings.run,
                        ...
                        '--env-replace-dir','/global/cfs/cdirs/cosmo/:/dummy_dir_for_testing/',';'] # AJRM Arnaud added option to change dir
