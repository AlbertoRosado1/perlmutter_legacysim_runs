...
def get_grid_in_brick(survey, brickname, rng=None, seed=None):
    ...
    #preprocess.py does not follow legacypipe conventions (bx, by should be shifted by -1). This does not impact the legacysim 
    # runs (based on ra/dec), but better have the correct column from the start
    catalog.bx, catalog.by = catalog.bx - 1, catalog.by - 1
...
if __name__ == '__main__':
    ...
    #parser.add_argument('-bfn','--bricklist_fn',type=str,required=True,help='bricklist filename')
    parser.add_argument('-i','--index',type=int,required=True,help='bricklist index?')
    ...
    output_dir = os.path.join(settings.output_dir,'file0_rs0_skip0')
    injected_fn = os.path.join(output_dir, f'injected_{opt.index}.fits')
    
    bricklist_fn = os.path.join('splitted_bricklists',f'bricklist_{opt.run}_{opt.index}.txt') #opt.briclist_fn
    #assert opt.run in bricklist_fn, "bricklist must be of same region, make sure if north or south"
    bricknames = [brickname[:-len('\n')] for brickname in open(bricklist_fn,'r')]

    if 'injected' in opt.do:
        injected = get_grid_randoms(settings.truth_fn,bricknames=bricknames,south=south,seed=42) 
        #injected = 0
